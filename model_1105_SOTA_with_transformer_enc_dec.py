...
class Encoder(nn.Module):
    def __init__(self, config: const.Config, embedding: nn.modules.sparse.Embedding) -> None:
        ...
        encoder_layers = TransformerEncoderLayer(d_model=100, nhead=10, dim_feedforward=200, dropout=0.1)
...
class Decoder(nn.Module):
    def __init__(self, config: const.Config, embedding: nn.modules.sparse.Embedding, device) -> None:
        ...
        self.src_mask = None
        #self.pos_encoder = PositionalEncoding(ninp, dropout)
        decoder_layers = TransformerDecoderLayer(d_model=1000, nhead=10, dim_feedforward=2000, dropout=0.1)
    ...
    def _decode_step(self, rnn_cell: nn.modules,
        ...
        return (predict_logits, copy_logits), decoder_state, output
...
class OneDecoder(Decoder):
    ...
    def forward(self, sentence: torch.Tensor, decoder_state: torch.Tensor, encoder_outputs: torch.Tensor) \
        ...
        for t in range(self.decodelen):
            ...
            bag, decoder_state, raw_output = self._decode_step(self.rnn, output, decoder_state, encoder_outputs, first_entity_mask)
        ...
        temp = torch.zeros(self.config.batch_size, self.config.max_sentence_length - self.config.decoder_output_max_length, self.config.decoder_num_units).to(self.device)
        raw_outputs = torch.stack(raw_outputs, dim=1)
        raw_outputs = torch.cat((raw_outputs,temp),dim=1)
        outputs = self.transformer_decoder(raw_outputs, encoder_outputs)
