...
class Decoder(nn.Module):
    def __init__(self, config: const.Config, embedding: nn.modules.sparse.Embedding, device) -> None:
        ...
        #--------------------------------------------------------------------jeong
        self.multihead_attn = torch.nn.MultiheadAttention(self.config.decoder_num_units, num_heads=10)
        #--------------------------------------------------------------------------
    ...
    def _decode_step(self, rnn_cell: nn.modules,
        ...
        temp_zero = torch.zeros([self.config.batch_size, self.config.decoder_num_units]).to(self.device)
        '''
        if t != 0:
            for i in range(self.config.decoder_output_max_length - t):
                raw_outputs.append(temp_zero)
            raw_outputs = torch.stack(raw_outputs, dim=0)
            #self_context = self.calc_context(decoder_state_h, raw_outputs)
            multihead_attn = torch.nn.MultiheadAttention(self.config.decoder_num_units, num_heads=10).to(self.device)
            attn_output, attn_output_weights = multihead_attn(raw_outputs, raw_outputs, raw_outputs)
            attn_output = torch.unbind(attn_output, dim=0)
            #print('@@@@@@@@@@@@@@@@jeong_attn_output',attn_output.size())[15,100, 1000]
            attn_output = torch.stack(attn_output, dim=1)
            #print('@@@@@@@@@@@@@@@@jeong_attn_output',attn_output.size())[100,15,1000]
            #print('@@@@@@@@@@@@@@@@jeong_context',context.size())[100,1000]
            context = self.combine_inputs2(torch.cat((context, attn_output), dim=-1))
        '''
        ...
        # print('@@@@@@@@@@@jeong_output',output.size())[100,1000]
        raw_outputs = raw_outputs
        raw_outputs.append(output)

        for i in range(self.config.decoder_output_max_length - (t+1)):
            raw_outputs.append(temp_zero)
        raw_outputs_for_attn = torch.stack(raw_outputs, dim=1)
        #print('@@@@@@@@jeong_raw_outputs_attn', raw_outputs_for_attn.size())
        attn_mask = self.get_attn_subsequent_mask(raw_outputs_for_attn).to(self.device)
        attn_mask = attn_mask.unbind(dim=1)
        #print('@@@@@@@@jeong_attn_mask', attn_mask.size())
        raw_outputs_stack = torch.stack(raw_outputs, dim=0)
        #print('@@@@@@@@jeong_raw_outputs_stack',raw_outputs_stack.size())


        attn_output, attn_output_weights = self.multihead_attn(raw_outputs_stack, raw_outputs_stack, raw_outputs_stack, key_padding_mask=attn_mask[t])
        attn_output = torch.unbind(attn_output, dim=0)

        #print('@@@@@@@@@@@jeong_attn_output',attn_output.size())
        output = attn_output[t]
        ...
        return (predict_logits, copy_logits), decoder_state, output_for_return
...
class OneDecoder(Decoder):
    ...
    def forward(self, sentence: torch.Tensor, decoder_state: torch.Tensor, encoder_outputs: torch.Tensor) \
        ...
        for t in range(self.decodelen):
            bag, decoder_state, raw_output = self._decode_step(self.rnn, output, decoder_state, encoder_outputs, first_entity_mask, raw_outputs ,t)
            #print('@@@@@@@@@@@@@jeong_raw_output', raw_output.size())[100.1000]
