...
class Decoder(nn.Module):
    ...
    def extract_relation_ver2(self, decoder_state: torch.Tensor, encoder_outputs: torch.Tensor) -> torch.Tensor:
        ...
        #print('@@@jeong_conv_emb', conv_emb.size())  # [100, 10, 1]
        conv_emb = conv_emb.squeeze(dim=-1)
        #print('@@@jeong_conv_emb',conv_emb.size()) #[100, 10]
        conv_emb = F.relu(conv_emb)
        pred_logit = self.do_rel_predict(conv_emb)
        eos_logit = self.do_rel_eos(conv_emb)

        return pred_logit, eos_logit

    def extract_relation_ori(self, decoder_state: torch.Tensor, encoder_outputs: torch.Tensor) -> torch.Tensor:
        ...
        conv_emb = attned_encoder_output.unsqueeze(dim=1)  # [100,1,80,80]
        conv1 = torch.nn.Conv1d(in_channels=1, out_channels=10, kernel_size=(3, self.config.max_sentence_length),
                                stride=1).to(self.device)
        conv_emb = conv1(conv_emb)  # [100,10,76,1]
        conv_emb = F.relu(conv_emb)
        conv_emb = conv_emb.squeeze(dim=-1)  # [100,10,76]
        conv_emb = conv_emb.unbind(dim=1)
        conv_emb = torch.cat(conv_emb, dim=-1)  # [100,76*10]\
        #print('@@@jeong_conv_emb',conv_emb.size()) #[100, 78, 10]

        do_predict = nn.Linear(conv_emb.size()[-1], self.relation_number).to(self.device)
        do_eos = nn.Linear(conv_emb.size()[-1], 1).to(self.device)

        pred_logit = do_predict(conv_emb)
        #print('@@@jeong_rel_pred_log',pred_logit)
        eos_logit = do_eos(conv_emb)
        #print('@@@jeong_rel_pred_log', eos_logit)
    ...
    def do_copy(self, output: torch.Tensor, encoder_outputs: torch.Tensor) -> torch.Tensor:
        ...
        #out = F.relu(self.fuse(F.relu(out)))
