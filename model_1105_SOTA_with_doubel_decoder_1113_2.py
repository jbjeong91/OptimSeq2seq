...
class OneDecoder(Decoder):
    ...
    def forward(self, sentence: torch.Tensor, decoder_state: torch.Tensor, encoder_outputs: torch.Tensor) \
        ...
        for t in range(self.decodelen):
            # lower layer ---------------------------------------------------------------------------------------------
            bag, decoder_state_lower = self._decode_step_lower(self.rnn, output, decoder_state_lower, encoder_outputs, first_entity_mask, t)
            predict_logits, copy_logits = bag
            ...
            # next time step
            if t % 3 == 0:
                output_low = max_action
                rel_output = self.relation_embedding(output_low)
