...
class OneDecoder(Decoder):
    ...
    def forward(self, sentence: torch.Tensor, decoder_state: torch.Tensor, encoder_outputs: torch.Tensor) \
        ...
        conv1 = torch.nn.Conv1d(in_channels=1, out_channels=50, kernel_size=(50, 50), stride=(1, 50)).to(self.device)
        #conv2 = torch.nn.Conv1d(in_channels=5, out_channels=1, kernel_size=(1, 1), stride=1).to(self.device)
        conv_emb = conv1(conv_emb)
        #print('@@@@@@@@@jeong_emb2', conv_emb.size()) #[100,50,31,20]
        conv_emb = F.relu(conv_emb)
        #print('@@@@@@@@@jeong_emb3', conv_emb.size())  # [100,50,31,20]
