    ...
    def _decode_step(self, rnn_cell: nn.modules,
        ...
        if t != 0:
            ...
            for i in range(self.config.decoder_output_max_length - (t + 1)):
                dec_states.append(temp_zero)
            dec_states_for_mask = torch.stack(dec_states, dim=1)

            attn_mask = self.get_attn_subsequent_mask(dec_states_for_mask).to(self.device)
            attn_mask = attn_mask.unbind(dim=1)
            dec_states = torch.stack(dec_states, dim=0)


            attn_output, attn_output_weights = self.multihead_attn(dec_states, dec_states, dec_states, key_padding_mask=attn_mask[t])
            attn_output = torch.unbind(attn_output, dim=0)

            # print('@@@@@@@@@@@jeong_attn_output',attn_output.size())
            decoder_state_h = attn_output[t]
...
class OneDecoder(Decoder):
    ...
    def forward(self, sentence: torch.Tensor, decoder_state: torch.Tensor, encoder_outputs: torch.Tensor) \
        ...
        for t in range(self.decodelen):
            ...
            raw_outputs.append(decoder_state[0].squeeze(0))
            #print('@@@@@@@@@@jeong_raw_outpout', decoder_state[0].squeeze(0).size())
