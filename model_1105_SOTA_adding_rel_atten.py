...
class OneDecoder(Decoder):
    ...
    def forward(self, sentence: torch.Tensor, decoder_state: torch.Tensor, encoder_outputs: torch.Tensor) \
        ...
        #print('@@@@@@@@@jeong_conv_emb', conv_emb.size()) #[100,80,1000]
        conv = torch.nn.Conv1d(in_channels=self.config.max_sentence_length, out_channels=self.config.max_sentence_length, kernel_size=5, stride=1, padding=2).to(self.device)
        conv_emb = conv(conv_emb)
        #print('@@@@@@@@@jeong_conv_emb2', conv_emb.size()) #[100,80,1000]
        max_pooling = torch.nn.MaxPool1d(3, stride=1, padding=1)
        conv_emb = max_pooling(conv_emb)
        #print('@@@@@@@@@jeong_conv_emb3', conv_emb.size())  # [100,80,1000]
        #--------------------------------------------------------------------------------------------------------
