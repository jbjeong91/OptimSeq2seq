...
class Encoder(nn.Module):
    def __init__(self, config: const.Config, embedding: nn.modules.sparse.Embedding) -> None:
        ...
        self.combine_inputs = nn.Linear(self.hidden_size + self.hidden_size, self.hidden_size)
    ...
    def forward(self, sentence: torch.Tensor, lengths: List[int]) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.cell_name == 'gru':
            ...
            hidden = tuple(map(lambda state: self.combine_inputs(torch.cat((torch.split(state, 1, dim=0)), dim=-1)), hidden))
        # hidden = (lambda a: sum(a)/2)(torch.split(hidden, 1, dim=0))
...
class Decoder(nn.Module):
    def __init__(self, config: const.Config, embedding: nn.modules.sparse.Embedding, device) -> None:
        ...
        self.sos_embedding2 = nn.Embedding(1, config.embedding_dim*2)
        ...
        #-----------------------------------------------------------------------------------
        self.conv = nn.Conv1d(in_channels=self.config.max_sentence_length, out_channels=self.config.max_sentence_length, kernel_size=5, stride=1, padding=2).to(self.device)
        self.max_pooling = torch.nn.MaxPool1d(3, stride=1, padding=1)
        #------------------------------------------------------------------------------------
        if self.cell_name == 'gru':
    ...
    def _decode_step2(self, rnn_cell: nn.modules,
                     emb: torch.Tensor,
        ...
        if cmd == 0:
            output, decoder_state = rnn_cell(output.unsqueeze(1))
        else:
    ...
    def _decode_step(self, rnn_cell: nn.modules,
        ...
        context = self.calc_context(decoder_state_h, encoder_outputs)
        #print('@@jeong_context:',context.size())
        if t != 0:
            context = context.unsqueeze(dim=0)
            dec_states = torch.stack(dec_states, dim=1)
            #print('@@jeong_dec_states2:',dec_states.size())
            context = self.calc_context(context, dec_states)

        output = self.combine_inputs(torch.cat((emb, context), dim=1))

        output, decoder_state = rnn_cell(output.unsqueeze(1), decoder_state)

        #print('@@jeong_decoder_state',decoder_state[0].size())
        #jeong->
        output = output.squeeze()
    ...
    def _decode_step_origin(self, rnn_cell: nn.modules,
                     emb: torch.Tensor,
                     decoder_state: torch.Tensor,
                     encoder_outputs: torch.Tensor,
                     first_entity_mask: torch.Tensor) \
...
class OneDecoder(Decoder):
    ...
    def forward(self, sentence: torch.Tensor, decoder_state: torch.Tensor, encoder_outputs: torch.Tensor) \
        ...
        conv_emb = conv_emb.unsqueeze(dim=1)
        #print('@@@@@@@@@jeong_emb1', conv_emb.size()) #[100,1,80,1000]
        conv1 = torch.nn.Conv1d(in_channels=1, out_channels=5, kernel_size=(50, 1), stride=1).to(self.device)
        conv2 = torch.nn.Conv1d(in_channels=5, out_channels=1, kernel_size=(1, 1), stride=1).to(self.device)
        conv_emb = conv1(conv_emb)
        #print('@@@@@@@@@jeong_emb2', conv_emb.size()) #[100,5,31,1000]
        conv_emb = F.relu(conv_emb)
        #print('@@@@@@@@@jeong_emb3', conv_emb.size())  # [100,5,31,1000]
        #max_pooling = torch.nn.MaxPool2d(kernel_size=(2,1,1), stride=1)
        #conv_emb = max_pooling(conv_emb)
        #print('@@@@@@@@@jeong_emb3_2', conv_emb.size())  # [100,5,31,1000]
        conv_emb = conv2(conv_emb)
        #print('@@@@@@@@@jeong_emb4', conv_emb.size())  # [100,1,31,1000]
        conv_emb = F.relu(conv_emb)
        #print('@@@@@@@@@jeong_emb5', conv_emb.size())  # [100,1,31,1000]
        conv_emb = conv_emb.squeeze(1)
        #print('@@@@@@@@@jeong_emb6', conv_emb.size())  # [100,31,1000]
        max_pooling = torch.nn.MaxPool2d((2,1), stride=(2,1))
        conv_emb = max_pooling(conv_emb)
        #print('@@@@@@@@@jeong_emb7', conv_emb.size())  # [100,15,1000]
        #--------------------------------------------------------------------------------------------------------

        for t in range(self.decodelen):
            #----------------------------- target_encoder---------------------
            if t == 0:
                decoder_state2 = decoder_state
            bag2, decoder_state2 = self._decode_step2(self.rnn, output, decoder_state2, encoder_outputs, t)
            predict_logits2, copy_logits2 = bag2

            if t % 3 == 0:
                action_logits2 = predict_logits2
            else:
                action_logits2 = copy_logits2

            max_action2 = torch.argmax(action_logits2, dim=1).detach()

            # next time step
            if t % 3 == 0:
                output2 = max_action2
                output2 = self.relation_embedding(output2)
            elif t%3==1:
                copy_index2 = torch.zeros_like(sentence).scatter_(1, max_action2.unsqueeze(1), 1).to(torch_bool)
                output2 = sentence[copy_index2]
                # output = self.word_embedding(output)
                # jeong_1 ->
                output2 = torch.cat((rel_output, self.word_embedding(output2)), dim=-1)
                output2 = self.combine_outputs(output2)
            else:
                copy_index2 = torch.zeros_like(sentence).scatter_(1, max_action2.unsqueeze(1), 1).to(torch_bool)
                output2 = sentence[copy_index2]
                output2 = self.word_embedding(output2)


            #-----------------------------------decoder -------------------------------
            #output = (output + output2) / 2
            output = self.combine_outputs(torch.cat((output, output2),dim=-1))
            bag, decoder_state = self._decode_step_new(self.rnn, output, decoder_state, encoder_outputs, first_entity_mask, conv_emb, t)
            predict_logits, copy_logits = bag
