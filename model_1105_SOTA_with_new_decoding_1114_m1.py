...
class Decoder(nn.Module):
    def __init__(self, config: const.Config, embedding: nn.modules.sparse.Embedding, device) -> None:
        ...
        self.pos_embedding = nn.Embedding(2, self.emb_size)
    ...
    def _decode_step_lower(self, rnn_cell: nn.modules,
                     emb: torch.Tensor,
                     decoder_state: torch.Tensor,
                     encoder_outputs: torch.Tensor,
                     first_entity_mask: torch.Tensor,
...
class OneDecoder(Decoder):
    ...
    def forward(self, sentence: torch.Tensor, decoder_state: torch.Tensor, encoder_outputs: torch.Tensor) \
        ...
        go = torch.zeros(sentence.size()[0], dtype=torch.int64).to(self.device)
        head = torch.zeros(sentence.size()[0], dtype=torch.int64).to(self.device)
        tail = torch.ones(sentence.size()[0], dtype=torch.int64).to(self.device)
        output = self.sos_embedding(go)
        rel_output = None  # jeong_1

        first_entity_mask = torch.ones(go.size()[0], self.maxlen).to(self.device)
        decoder_state_lower = decoder_state
        for t in range(self.decodelen):
            # lower layer ---------------------------------------------------------------------------------------------
            bag_low, decoder_state_lower = self._decode_step(self.rnn, output, decoder_state_lower, encoder_outputs, first_entity_mask)
            predict_logits_low, copy_logits_low = bag_low

            if t % 3 == 0:
                action_logits_low = predict_logits_low
            else:
                action_logits_low = copy_logits_low

            max_action_low = torch.argmax(action_logits_low, dim=1).detach()
            ...
            # next time step
            if t % 3 == 0:
                output_low = max_action_low
                rel_output_low = self.relation_embedding(output_low)
                output_low = self.relation_embedding(output_low)
            elif t%3 ==1:
                copy_index = torch.zeros_like(sentence).scatter_(1, max_action_low.unsqueeze(1), 1).to(torch_bool)
                output_low = sentence[copy_index]
                head_output_low = self.word_embedding(output_low) + self.pos_embedding(head)
                output_low = self.word_embedding(output_low)
                # jeong_1 ->
                #output_low = torch.cat((rel_output, self.word_embedding(output_low)), dim=-1)
                #output_low = self.combine_outputs(output_low)
            else:
                copy_index = torch.zeros_like(sentence).scatter_(1, max_action_low.unsqueeze(1), 1).to(torch_bool)
                output_low = sentence[copy_index]
                # jeong_1 ->
                #output_low = self.word_embedding(output_low)
                #output_low = ((rel_output_low - head_output_low) + (output_low + self.pos_embedding(tail))) / 2
                output_low = torch.cat((rel_output, self.word_embedding(output_low)), dim=-1)
                output_low = self.combine_outputs(output_low)
            ...
            if t % 3 == 1:
                ...
                index = torch.zeros_like(first_entity_mask).scatter_(1, max_action_low.unsqueeze(1), 1).to(torch_bool)
            ...
            # higher layer ---------------------------------------------------------------------------------------------
            #output = self.combine_outputs(torch.cat((output, output_low), dim=-1))
            bag, decoder_state = self._decode_step(self.rnn, output, decoder_state, encoder_outputs, first_entity_mask)
            predict_logits, copy_logits = bag

            if t % 3 == 0:
                #action_logits = predict_logits
                action_logits = (predict_logits + predict_logits_low)/2
            else:
                #action_logits = copy_logits
                action_logits = (copy_logits + copy_logits_low)/2
