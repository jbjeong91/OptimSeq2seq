...
class OneDecoder(Decoder):
    ...
    def forward(self, sentence: torch.Tensor, decoder_state: torch.Tensor, encoder_outputs: torch.Tensor) \
        ...
        conv1_1 = torch.nn.Conv1d(in_channels=1, out_channels=3, kernel_size=(50, 1), stride=1).to(self.device)
        conv1_2 = torch.nn.Conv1d(in_channels=1, out_channels=3, kernel_size=(30, 1), stride=1).to(self.device)
        conv1_3 = torch.nn.Conv1d(in_channels=1, out_channels=3, kernel_size=(10, 1), stride=1).to(self.device)
        conv_emb1_1 = F.relu(conv1_1(conv_emb))
        #print('@@@@@@@@@jeong_emb2', conv_emb1_1.size())  # [100,3,31,1000]
        conv_emb1_2 = F.relu(conv1_2(conv_emb))
        conv_emb1_3 = F.relu(conv1_3(conv_emb))
        #max_pooling1 = torch.nn.MaxPool2d(kernel_size=((self.config.max_sentence_length-50+1)//5,1), stride=((self.config.max_sentence_length-50+1)//5, 1))
        #max_pooling2 = torch.nn.MaxPool2d(kernel_size=((self.config.max_sentence_length-30+1)//5,1), stride=((self.config.max_sentence_length-30+1)//5, 1))
        #max_pooling3 = torch.nn.MaxPool2d(kernel_size=((self.config.max_sentence_length-10+1)//5,1), stride=((self.config.max_sentence_length-10+1)//5, 1))
        #conv_emb1_1 = max_pooling1(conv_emb1_1)
        #print('@@@@@@@@@jeong_emb3', conv_emb1_1.size())  # [100,3,5,1000]
        #conv_emb1_2 = max_pooling2(conv_emb1_2)
        #conv_emb1_3 = max_pooling3(conv_emb1_3)
        conv2_1 = torch.nn.Conv1d(in_channels=3, out_channels=1, kernel_size=((self.config.max_sentence_length-50+1)//5,1), stride=1).to(self.device)
        conv2_2 = torch.nn.Conv1d(in_channels=3, out_channels=1, kernel_size=((self.config.max_sentence_length-30+1)//5,1), stride=1).to(self.device)
        conv2_3 = torch.nn.Conv1d(in_channels=3, out_channels=1, kernel_size=((self.config.max_sentence_length-10+1)//5,1), stride=1).to(self.device)
        conv_emb1_1 = F.relu(conv2_1(conv_emb1_1))
        #print('@@@@@@@@@jeong_emb4', conv_emb1_1.size())  # [100,1,5,1000]
        conv_emb1_2 = F.relu(conv2_2(conv_emb1_2))
        conv_emb1_3 = F.relu(conv2_3(conv_emb1_3))
        conv_emb1_1 = conv_emb1_1.squeeze(dim=1)
        conv_emb1_2 = conv_emb1_2.squeeze(dim=1)
        conv_emb1_3 = conv_emb1_3.squeeze(dim=1)
        conv_emb = torch.cat((conv_emb1_1, conv_emb1_2), dim=1)
        conv_emb = torch.cat((conv_emb, conv_emb1_3), dim=1)
        #print('@@@@@@@@@jeong_emb4', conv_emb.size())  # [100,1,5,1000]
        #--------------------------------------------------------------------------------------------------------
