...
class CapsNet(nn.Module):
    def __init__(self, config: const.Config, routing_iterations, device):
        ...
        self.n_classes = config.relation_number
        self.device = device
        ...
        self.primaryCaps = PrimaryCapsLayer(input_channels=256, output_caps=32, output_dim=8, kernel_size=10, stride=(6,5), device=device)  # outputs 6*6
        self.num_primaryCaps = 32*9*6#32 * 6 * 6
        routing_module = AgreementRouting(self.num_primaryCaps, self.n_classes, routing_iterations, device)
        self.digitCaps = CapsLayer(self.num_primaryCaps, 8, self.n_classes, 16, routing_module, device)
        '''
...
class MultiDecoder(Decoder):
    ...
    def forward(self, sentence: torch.Tensor, decoder_state: torch.Tensor, encoder_outputs: torch.Tensor) \
        ...
        conv_emb = encoder_outputs
        #print('@@@@@@@@@jeong_basic_emb', basic_emd.size()) #[100,80,1000]
        conv = torch.nn.Conv1d(in_channels=self.config.max_sentence_length, out_channels=self.relation_number, kernel_size=5, stride=1, padding=2).to(self.device)
        conv_emb = conv(conv_emb)
        print('@@@@@@@@@jeong_conv_emb',conv_emb.size())
        max_pooling = torch.nn.MaxPool1d(3, stride=1, padding=1)
        conv_emb = max_pooling(conv_emb)
...
class OneDecoder(Decoder):
    ...
    def forward(self, sentence: torch.Tensor, decoder_state: torch.Tensor, encoder_outputs: torch.Tensor) \
        ...
        #print('@@@@@@@@@jeong_cabs_emb',caps_emb)
        #--------------------------------------------------------------------------------------------------------
        for t in range(self.decodelen):

            bag, decoder_state = self._decode_step_rel_attn(self.rnn, output, decoder_state, encoder_outputs, first_entity_mask, cabs_prob)
            predict_logits, copy_logits = bag
        ...
        return pred_action_list, pred_logits_list, caps_result
...
class Seq2seq(nn.Module):
    ...
    def forward(self, sentence: torch.Tensor, sentence_eos: torch.Tensor, lengths: List[int]) \
        ...
        pred_action_list, pred_logits_list, caps_result = self.decoder(sentence=sentence_eos, decoder_state=h, encoder_outputs=o)
