...
class SupervisedTrainer(object):
    ...
    def train_step(self, batch: data_prepare.InputData) -> torch.Tensor:
        ...
        #-----------------------------------------------------------------------jeong_for_capsnet_reconstruct
        #print('@@@@jeong_triple',triplets.size())#[100,15] = [batch_, dec_len]
        temp_tripets = triplets.unbind(triplets, dim=1)
        rel_target = []
        for t, item in enumerate(temp_tripets):
            if t%3==0:
                rel_target.append(t)
        rel_target = torch.stack(rel_target, dim=1)
        #print('@@@@jeong_rel_target', rel_target.size())
        #-----------------------------------------------------------------------
        sentence = torch.from_numpy(sentence).to(self.device)
        sentence_eos = torch.from_numpy(sentence_eos).to(self.device)
        ...
        pred_action_list, pred_logits_list, caps_result = self.seq2seq(sentence, sentence_eos, lengths)
        loss = 0
        caps_loss_fn = MarginLoss(0.9, 0.1, 0.5)
        for t in range(self.seq2seq.decoder.decodelen):
            c_loss = caps_loss_fn(caps_result[1], rel_target)
            loss = loss + self.loss(pred_logits_list[t], triplets[:, t]) + c_loss
    ...
    def train(self, evaluator: Evaluator=None) -> None:
        ...
        for epoch in range(1, self.epoch_number + 1):
            ...
            if evaluator:
                ...
                print('_' * 60)
                print('{} seconds'.format(time.time()))
