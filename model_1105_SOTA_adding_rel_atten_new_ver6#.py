...
class Decoder(nn.Module):
    ...
    def calc_context(self, decoder_state: torch.Tensor, encoder_outputs: torch.Tensor) -> torch.Tensor:
        ...
        print('###########jeong_attn_wegiht',attn_weight.size()) #[100,80,2000]
        attn_weight = F.softmax((self.attn(attn_weight)), dim=1)
        print('###########jeong_attn_wegiht', attn_weight.size()) #[100,80,1]
        attn_applied = torch.bmm(attn_weight.permute(0, 2, 1), encoder_outputs).squeeze(1) #[100,1000]
        return attn_applied
    ...
    def extract_relation(self, decoder_state: torch.Tensor, encoder_outputs: torch.Tensor) -> torch.Tensor:
        ...
        conv1 = torch.nn.Conv1d(in_channels=1, out_channels=10, kernel_size=(3, self.config.max_sentence_length), stride=1).to(self.device)
        conv_emb = conv1(conv_emb) #[100,10,76,1]
        conv_emb = F.relu(conv_emb)
        conv_emb = conv_emb.squeeze(dim=-1) #[100,10,76]
        conv_emb = conv_emb.unbind(dim=1)
        conv_emb = torch.stack(conv_emb, dim=-1) #[100,76*10]
        distributed_rel = nn.Linear(conv_emb.size()[-1], self.relation_number)
        conv_logits = distributed_rel(conv_emb)
    ...
    def _decode_step_new(self, rnn_cell: nn.modules,
        ...
        context = self.calc_context_for_rel(decoder_state_h, encoder_outputs)
...
class OneDecoder(Decoder):
    ...
    def forward(self, sentence: torch.Tensor, decoder_state: torch.Tensor, encoder_outputs: torch.Tensor) \
        ...
        conv1 = torch.nn.Conv1d(in_channels=1, out_channels=10, kernel_size=(30, 5), stride=(1, 5)).to(self.device)
        conv2 = torch.nn.Conv1d(in_channels=10, out_channels=5, kernel_size=(20, 1), stride=1).to(self.device)
        conv_emb = conv1(conv_emb)
        #print('@@@@@@@@@jeong_emb2', conv_emb.size()) #[100,10,51,200]
        conv_emb = F.relu(conv_emb)
        #max_pooling = torch.nn.MaxPool2d((2,1), stride=(2,1))
        #conv_emb = max_pooling(conv_emb)
        #print('@@@@@@@@@jeong_emb3', conv_emb.size())  # [100,10,51,200]
        conv_emb = conv2(conv_emb)
        conv_emb = F.relu(conv_emb)
        conv_emb = torch.unbind(conv_emb, dim=1)
        conv_emb = torch.cat(conv_emb, dim=-1)
        #print('@@@@@@@@@jeong_emb6', conv_emb.size())  # [100,32,1000]
        #--------------------------------------------------------------------------------------------------------
